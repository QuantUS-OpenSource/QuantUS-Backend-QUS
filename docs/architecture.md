...
4. **Analysis** (`quantus/analysis/`) — QUS methods as plugins under `paramap/analysis_methods/` (or `bmode/analysis_methods/`), either a single `.py` file (the common case) or a folder with `__init__.py` (for a plugin that needs to bundle a sibling data asset, e.g. `homodyned_k_params/` bundling its `RSK_data_501.mat` lookup table) — discovered identically either way. Most are **window**-scoped: a function taking `(scan_rf_window, phantom_rf_window, window, config, image_data, **kwargs)` and writing results onto that one window's `window.results`, with no visibility into any other window. An **`aggregate`**-located function (declared via the `location` decorator) instead runs once, after every window has been processed, receiving the full window list and writing onto a separate aggregate results object — for computations that genuinely need every window's result at once (e.g. averaging a per-window curve across the ROI before a single scan-wide curve fit). Aggregate functions live in a sibling `paramap/analysis_aggregators/` (or `bmode/analysis_aggregators/`) folder rather than inside `analysis_methods/`, so that a folder under `analysis_methods/` always means one plugin, never a category of several. See [quantus/analysis/README.md](../quantus/analysis/README.md) for the full contract, including the third `full_segmentation` location.
