    ...
    def __init__(self, analysis_obj: ParamapAnalysisBase, **kwargs):
        ...
        if self.analysis_obj.image_data.sc_bmode is not None:
            ...
            if image_data.sc_bmode.ndim == 2:
                ...
                self.sc_window_idx_map = np.array(scanConvert3dVolumeSeries(self.window_idx_map, image_data.sc_params_3d, scale=False, interp='nearest', normalize=False)[0],
                                                  dtype=int)
            else:
                raise NotImplementedError("Scan conversion only implemented for 2D and 3D data.")
            
        
    def draw_paramap(self, param: str, cmap: list) -> Tuple[np.ndarray, plt.Figure, np.ndarray, np.ndarray]:
        ...
        colored_paramap = np.zeros(idx_map.shape + (4,), dtype=np.uint8); numerical_paramap = np.full(idx_map.shape, np.nan, dtype=np.float32)
        windowed_paramap_mask = np.zeros(idx_map.shape, dtype=bool)
        skipped_indices = []
        for point in window_points:
            if int(np.round(idx_map[tuple(point)])) in skipped_indices:
                continue
            window = self.analysis_obj.windows[int(np.round(idx_map[tuple(point)])-1)]
            try:
                color_ix = int((255 / (max_val-min_val)
                                     )*(getattr(window.results, param)-min_val)
                                    ) if min_val != max_val else 125
            except (ValueError, OverflowError):
                print(f"Parameter '{param}' not computed properly for window {int(np.round(idx_map[tuple(point)]))}. Skipping...")
                skipped_indices.append(int(np.round(idx_map[tuple(point)])))
                continue
            numerical_paramap[tuple(point)] = getattr(window.results, param)
            windowed_paramap_mask[tuple(point)] = True
            colored_paramap[tuple(point)][:3] = (np.array(cmap[color_ix])*255).astype(np.uint8)
            colored_paramap[tuple(point)][3] = 255
        ...
        return colored_paramap, fig, numerical_paramap, windowed_paramap_mask
