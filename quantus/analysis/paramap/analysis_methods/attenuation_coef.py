...
@output_vars("att_coefs", "att_subf")
...
def attenuation_coef(scan_rf_window: np.ndarray, phantom_rf_window: np.ndarray, 
    ...
    roi_len = (scan_rf_window.shape[0]-1) * image_data.axial_res
    start_depth = window.ax_min * image_data.axial_res
    sec_len = roi_len / ((1-sec_overlap)*(n_secs-1) + 1)

    sec_start_indices = []; sec_end_indices = []
    for i in range(n_secs):
        sec_start_depth = start_depth + i*(1-sec_overlap)*sec_len
        sec_end_depth = start_depth + i*(1-sec_overlap)*sec_len + sec_len
        sec_start_indices.append(np.ceil(sec_start_depth/image_data.axial_res)-window.ax_min)
        sec_end_indices.append(np.floor(sec_end_depth/image_data.axial_res)-window.ax_min)
    sec_start_indices = np.array(sec_start_indices).astype(int)
    sec_end_indices = np.array(sec_end_indices).astype(int)
    ...
    for i, (sec_start_ix, sec_end_ix) in enumerate(zip(sec_start_indices, sec_end_indices)):
        sub_window_rf = scan_rf_window[sec_start_ix:sec_end_ix+1]
        f, ps = compute_power_spec(sub_window_rf, start_frequency, end_frequency, sampling_frequency, n_fft, use_hanning=False)
        ps_sample[i] = ps
        
        ref_sub_window_rf = phantom_rf_window[sec_start_ix:sec_end_ix+1]
        _, ref_ps = compute_power_spec(ref_sub_window_rf, start_frequency, end_frequency, sampling_frequency, n_fft, use_hanning=False)
        ps_ref[i] = ref_ps
        
        start_depth = (sec_start_ix + window.ax_min)*image_data.axial_res
        sec_start_depths.append(start_depth)
        
    half_sec_len = (sec_end_indices[0] - sec_start_indices[0])*image_data.axial_res/2
    depths = (sec_start_depths + half_sec_len)/10
    spectral_ratios = np.log(ps_sample/ps_ref)
    spectral_ratios = np.nan_to_num(spectral_ratios, 1)
    ...
    window.results.att_coefs = att   # db/cm
    window.results.att_subf = f*1e-6 # MHz
