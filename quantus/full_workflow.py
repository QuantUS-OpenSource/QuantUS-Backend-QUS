...
from .image_loading.utc_loaders.options import get_scan_loaders, scan_loader_args
from .seg_loading.options import get_seg_loaders, seg_loader_args
from .analysis_config.utc_config.options import get_config_loaders, config_loader_args
from .analysis.options import get_analysis_types, analysis_args
from .visualizations.options import get_visualization_types, visualization_args
from .data_export.options import get_data_export_types, data_export_args
...
def main_cli() -> int:
    ...
    args.data_export_kwargs = json.loads(args.data_export_kwargs)
...
def core_pipeline(args) -> int:
    ...
    # Check config paths
    try:
        exts = config_loaders[args.config_loader].supported_extensions if hasattr(config_loaders[args.config_loader], 'supported_extensions') else []
        assertions = [args.config_path.endswith(ext) for ext in exts]
        assert not len(assertions) or max(assertions), f"Config file must end with {', '.join(exts)}"
    except KeyError:
    ...
    for name in args.analysis_funcs:
        ...
        analysis_kwargs = getattr(analysis_funcs[args.analysis_type][name], 'required_kwargs', [])
    ...
    for name in args.data_export_funcs:
        ...
        export_func = data_export_funcs[args.data_export_type][name]
        if hasattr(export_func, 'required_kwargs'):
            required_kwargs = export_func.required_kwargs
            for kwarg in required_kwargs:
                if kwarg not in args.data_export_kwargs:
                    raise ValueError(f"data_export_kwargs: Missing required keyword argument '{kwarg}' for function '{name}' in {args.data_export_type} data export type.")
    ...
    # Analysis
    # Check analysis functions support for data dimensionality
    all_analysis_funcs = analysis_funcs[args.analysis_type]
    for name in args.analysis_funcs:
        supported_spatial_dims = all_analysis_funcs[name].supported_spatial_dims if hasattr(all_analysis_funcs[name], 'supported_spatial_dims') else []
    ...
    data_export_obj = data_export_class(visualization_obj, args.data_export_path, args.data_export_funcs, **args.data_export_kwargs)
